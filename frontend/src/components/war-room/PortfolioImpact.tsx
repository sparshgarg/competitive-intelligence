...
export function PortfolioImpact({ initiatives, sliders }: PortfolioImpactProps) {
  const data = useMemo(() => {
    return initiatives.map((init) => {
      ...
      return {
        name: init.name.substring(0, 26) + (init.name.length > 26 ? "…" : ""),
  ...
  return (
    <Card className="p-5 glass-card h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-ink-2">Exposure ($M)</span>
      </div>
      ...
      <div className="h-[250px] w-full text-xs">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis type="number" tickFormatter={(val) => `$${val}M`} stroke={COLORS.ink3} fontSize={11} />
            <YAxis 
              dataKey="name" 
              type="category" 
              width={120} 
              stroke={COLORS.ink3}
              fontSize={11}
              tick={{ fill: COLORS.ink }}
            />
            <Tooltip 
              formatter={(value: number, name: string) => [`$${value.toFixed(1)}M`, name === 'adjusted' ? 'Adjusted Exposure' : 'Base Exposure']}
              contentStyle={{ borderRadius: '8px', border: `1px solid ${COLORS.border}`, backgroundColor: COLORS.surface }}
            />
            <Bar dataKey="adjusted" radius={[0, 4, 4, 0]} animationDuration={500}>
              {data.map((entry, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={entry.difference > 0.5 ? "#ef4444" : entry.difference < -0.5 ? "#22c55e" : COLORS.navy} 
                />
              ))}
            </Bar>
          </BarChart>
      ...
      <div className="flex gap-4 justify-center mt-4 text-xs text-ink-2">
        <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500"></div> Exposure Up</div>
        <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-navy"></div> Stable</div>
